...
const Penarikan = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.siswa_id || !formData.jumlah) return alert("Mohon lengkapi data penarikan!");
    ...
    if (selectedStudent && parseInt(formData.jumlah) > selectedStudent.saldo) {
        return alert(`❌ Saldo tidak cukup! Sisa saldo: Rp ${selectedStudent.saldo.toLocaleString('id-ID')}`);
    }
    setLoading(true);
      ...
      <div className="dashboard-card" style={{maxWidth: '600px', borderTop: '4px solid #C62828'}}>
        <form className="profile-form" onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <input type="number" name="jumlah" placeholder="Contoh: 100000" value={formData.jumlah} onChange={handleChange} required min="1000" style={{borderColor: '#C62828'}} />
          </div>
          <div className="form-group">
            <label>Keterangan</label>
            <input type="text" name="keterangan" placeholder="Contoh: Uang Saku / Beli Buku" value={formData.keterangan} onChange={handleChange} />
          </div>
          <div style={{marginTop: '20px'}}>
            <button type="submit" className="btn-submit" style={{width: '100%', padding: '15px', fontSize: '16px', opacity: loading ? 0.7 : 1, backgroundColor: '#C62828'}} disabled={loading}>{loading ? 'Memproses...' : 'TARIK SALDO'}</button>
          </div>
        </form>
      </div>
    </div>
